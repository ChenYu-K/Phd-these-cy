...
\begin{figure}[htbp]
    ...
    %\begin{subfigure}[t]{0.7\linewidth}
        \centering
        \includegraphics[width=0.7\textwidth]{imgs/ch2/rivet-cor-1.jpg}%rivet-cor.png
...
Where, $k_s$ is the reduction coefficient based on bolt hole size. $F_{p,C}$ is the preloading force for bolts, conforming to the requirements given in EN 14399 series\cite{EN14399},  with controlled tightening in accordance with the requirements given in 8.5 of EN 1090-2:2018 \cite{en1090-2-2018}, and given by: $N_{b} = 0.7 f_{ub} A_s$. $\gamma_{M3}$ is the partial factor for slip resistance at ultimate limit state (Category C).

For AASHTO LRFD BDS \cite{AASHTO2020}, the slip resistance is defined as:
\begin{equation}
    F_{s,AASHTO} = K_s \mu n N_b
\end{equation}

Where, $N_b$ is the same as Eurocode $= 0.7f_{ub}A_s$

The calculation of slip resistance is similar across different countries. However, there are significant differences in the values and treatment methods of the slip coefficient $\mu$ at the joint faying surface. The slip coefficients for faying surfaces specified by various countries are listed in Table \ref{tab-mucompare}.


\begin{table}[htbp]
\centering
\caption{Compare the slip coefficients and surface treatments specified in various national standards}
\label{tab-mucompare}
\begin{tabular}{@{}p{3cm}p{2.5cm}p{1.5cm}p{6cm}@{}}
\toprule
\textbf{Standard} & \textbf{\begin{tabular}[c]{@{}l@{}}Surface \\ Condition\end{tabular}} & \textbf{Minimum $\mu$} & \textbf{Surface Preparation Requirements} \\ \midrule
\multirow{2}{*}{\begin{tabular}[c]{@{}l@{}}AASHTO \\ LRFD (US)\end{tabular}} & Class A & 0.33 & Clean mill scale \\
 & Class B & 0.50 & Blast cleaned surfaces \\
\multirow{4}{*}{\begin{tabular}[c]{@{}l@{}}Eurocode 3 \\ (EN 1993-1-8)\end{tabular}} & Category A & 0.20 & Surfaces blasted with shot or grit with loose rust removed, not pitted \\
 & Category B & 0.30 & Surfaces blasted with shot or grit to Sa2½ \\
 & Category C & 0.40 & Surfaces roughened by sand blasting to Sa2½ and protected against surface rust by a zinc silicate coating \\
 & Category D & 0.40 & Surfaces roughened by sand blasting to Sa2½ and spray metallized with aluminum or zinc based product \\
\multirow{2}{*}{JSHB (Japan)} & Standard & 0.40 & Clean mill scale \\
 & Enhanced & 0.45 & Shot or grit blasted surfaces with special treatment \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{imgs/ch2/ch2figeu5-13.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\linewidth]{imgs/ch2/oneBbolt.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\linewidth]{imgs/ch2/cs-b1.pdf}
